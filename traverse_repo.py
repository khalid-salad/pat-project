...
TO = datetime(MAXYEAR, 12, 31, 23, 59, 59)
...
def get_data(
    ...
    print("commit_hash,file,commit date,committer,old path,new path,change type,number of major contributors,number of minor contributors,number of commits to file")
    ...
    for commit in Repository(repo_path, since=since, to=to).traverse_commits():
        for file in commit.modified_files:
            ...
            if any(["test" in x for x in [old_path, new_path]]):
                ...
                print(
                    f"{commit.hash},{file.filename},{commit.committer_date},{commit.committer.name},{file.old_path},{file.new_path},{file.change_type.name},{ct},{mt},{nc}"
                )
